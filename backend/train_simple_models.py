...
def train_all_simple_models():
    """Train simple models for all NSE stocks"""
    print("🚀 Training Simple Models for Realistic Predictions...")
    print("=" * 70)
    ...
    for i, symbol in enumerate(NSE_STOCKS, 1):
        print(f"\n[{i}/{len(NSE_STOCKS)}] 🎯 Training simple model for {symbol}...")
        
        try:
            # Create and train simple predictor
            ...
            if success:
                ...
                print(f"✅ Simple model trained successfully for {symbol}")
                ...
                print(f"❌ Simple training failed for {symbol}")
    ...
    with open('backend/simple_training_results.json', 'w') as f:
...
def test_simple_model(symbol: str):
    """Test a trained simple model"""
    try:
        ...
        if predictor.load_model(symbol):
            ...
            predictions, confidence = predictor.predict(df, current_price)
            
            print(f"\n🔍 Simple predictions for {symbol}:")
            ...
            print(f"❌ No simple model found for {symbol}")
            return None, None
            
    except Exception as e:
        print(f"❌ Error testing simple model for {symbol}: {e}")
        return None, None

if __name__ == "__main__":
    # Train all simple models
    results = train_all_simple_models()
